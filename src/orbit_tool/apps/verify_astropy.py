...
def add_astropy_cols(df:pandas.DataFrame):
    ...
    time = j2000.tai + astropy.time.TimeDelta(df['j2000_sec'], format="sec", scale="tai")
...
def build_data_frame(config:dict, context:DataContext) -> pandas.DataFrame:
    ...
    orekit_itrf = orekitfactory.get_frame("itrf", context=context, simpleEop=False, iersConventions="2010")
