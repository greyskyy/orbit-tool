...
from ..utils import read_orbit, OrbitType as OrbitTypeData, start_stop_step, orbit_to_dict
...
def config_args(parser):
    ...
    parser.add_argument(
        "--to",
        type=str,
        choices=[t.name.lower() for t in OrbitTypeData],
        default=OrbitTypeData.AUTO_SELECT.name.lower(),
...
def execute(args=None) -> int:
    ...
    dest_type = OrbitTypeData[args.to.upper()]
    ...
    # load the seed orbit
    seed_orbit, seed_type = read_orbit(
        orbit_name=args.seed_orbit, config=config["orbits"]
    )
    if not OrbitTypeData.compatible_with(seed_type, dest_type):
        logger.error(
    ...
    start_date, stop_date, step = start_stop_step(args, config, orbit.getDate())
    ...
    new_orbit = None
    if dest_type is OrbitTypeData.TLE:
    ...
    print("")
    print(json.dumps(orbit_to_dict(new_orbit, dest_type, config), indent=2))
    print("")
